...
module.exports = {
  ...
  async execute(interaction) {
    ...
    embed.addField(
      ...
      (() => {
        ...
        Object.entries(settings.features).forEach(([key, value]) => {
          ...
          if (value !== false) {
            general_settings += format.prettyPrint(
              `${key}: ${client.findEmoji('greenCheckBox')} ${highlighted(
                'enabled',
              )},`,
            );
          } else {
            ...
            general_settings += format.prettyPrint(
              `${key}: ${client.findEmoji('redCheckBox')} ${highlighted(
                'disabled',
              )},`,
            );
          }
        ...
        Object.entries(settings.roles)
          ...
          .forEach(([key, value]) => {
            ...
            if (key.includes('role') && value !== null) {
              role_settings += format.prettyPrint(
                `${key}: ${roleMention(value)},`,
              );
            } else {
              // If the role settings have not be set, display null
              role_settings += format.prettyPrint(
                `${key}: ${highlighted(value)},`,
              );
            }
          });
    ...
    const row = format.row().addComponents(
      // The edit button
      format
        .button()

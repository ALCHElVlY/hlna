  ...
  run: async (client, oldMember, newMember) => {
    ...
    if (rolesAdded.size > 0) {
      ...
      const channel = settings.log_channels.find(
        (c) => c.log_type === 'role_add',
      );
      const logChannel = channel
        ? client.channels.cache.get(channel.channel_id)
        : undefined;
      if (!logChannel) return;

      try {
        rolesAdded.forEach((r) => {
          logger.log([newMember, r], logChannel, 'role_add');
        });
      } catch (e) {
        console.log(e);
      }
    ...
    if (rolesRemoved.size > 0) {
      ...
      try {
        rolesRemoved.forEach((r) => {
          logger.log([newMember, r], logChannel, 'role_remove');
        });
      } catch (e) {
        console.log(e);
      }

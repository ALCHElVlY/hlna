...
const CLONE_EMBED = (
  ...
  const embed = format
    ...
    .setDescription(
      '**Creature**: ' +
        `${highlighted(creatureName.toProperCase())}` +
        '\n' +
        '**Level**: ' +
        `${highlighted(level)}`,
    )
    ...
    .addFields(
      ...
      {
        name: 'Total Time',
        value: `${client.findEmoji('stopwatch1')} ${client.calculate(
          clone_time,
        )}`,
        inline: false,
      },
...
const SERVER_EMBED = (data) => {
  const status = data.status.includes('offline')
    ? `${client.findEmoji('redCheckBox')} ${data.status}`
    : data.status.includes('online')
    ? `${client.findEmoji('greenCheckBox')} ${data.status}`
    : '';
